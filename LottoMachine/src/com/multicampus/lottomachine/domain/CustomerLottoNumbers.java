...
/** 사용자 입력을 처리하는 모델 
 * 중복없이 6개, 1~45범위 안에서만 받아야 함
 * 
 * */
// 구조가 비슷하면 extends, implement 써도 되나?
// 당첨번호 모델 WinningLottoNumbers - TreeSet , getter(public), setter(private)
// 당첨결과 모델 ResultLottoNumbers - Treeset, getter(public), setter(private)
public class CustomerLottoNumbers implements LottoNumbers{
	private TreeSet<Integer> customerNumbers = new TreeSet<Integer>();	//사용자 데이터를 입력할 TreeSet
	private String[] inputs;	//TODO 이름 바꿀 수 있으면 바꾸기
	
	public CustomerLottoNumbers(String input) {// 1,2,3,3,5,7
		inputs = input.split(",");		// 입력값을 ','을 기준으로 배열에 넣습니다.
		setNumbers();					// 배열의 값을 Treeset에 저장
	}
	
	public void setNumbers() throws NumberFormatException{	//사용자 값을 Treeset에 넣기
	...
	public TreeSet<Integer> getNumbers(){	//입력된 사용자 데이터 가져오기 
